...
class WebCrawler:
    ...
    def crawl_pages(self, base_url):
        """Crawl pages within a single domain (limit to 10 URLs)."""
        start_url = self.normalize_url(base_url)
        to_visit = [start_url]
        domain = urlparse(start_url).netloc

        while to_visit and len(self.visited) < 10:  # ✅ stop after 10 URLs
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Crawl finished. Saved {len(all_urls)} URLs to data/all_urls.json")
